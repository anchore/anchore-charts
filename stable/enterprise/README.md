...
Each key must be a 32-byte value, base64url-encoded. Generate one with:
...
stringData:
  ...
  # ... other ANCHORE_* env vars
  ANCHORE_DB_ENCRYPTION_KEY_CURRENT: "REPLACE_WITH_GENERATED_KEY"
...
```yaml
global:
  imageRegistryHost: harbor.example.com/anchore-licensed   # image, ui.image

kubectlImage:
  registry: harbor.example.com/dockerhub
scratchVolume:
  fixerInitContainerImage:
    registry: harbor.example.com/dockerhub
cloudsql:
  image:
...
| Name                                               | Description                                                                                                                                                                                                                                                  | Value                                                                     |
| -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------- |
| `image.registry`                                   | Registry for the Anchore Enterprise image. Empty means take it from global.imageRegistryHost; set it to pull this one image from a different registry                                                                                                        | `""`                                                                      |
| `image.repository`                                 | Repository for the Anchore Enterprise image                                                                                                                                                                                                                  | `anchore/enterprise`                                                      |
| `image.digest`                                     | Digest for the Anchore Enterprise image. Used when image.tag is not set                                                                                                                                                                                      | `sha256:19339361a617e4a9dbdd466ce4ea23958ea98160ee61e1f3cdfb56fbeb0052bc` |
| `image.tag`                                        | Tag for the Anchore Enterprise image. Set it to deploy a specific version instead of the digest the chart pins; quote the value so it is read as a string                                                                                                    | `""`                                                                      |
| `imagePullPolicy`                                  | Image pull policy used by all deployments                                                                                                                                                                                                                    | `IfNotPresent`                                                            |
| `imagePullSecretName`                              | Name of Docker credentials secret for access to private repos                                                                                                                                                                                                | `anchore-enterprise-pullcreds`                                            |
| `kubectlImage.registry`                            | Registry for the kubectl image. Empty means take it from global.imageRegistryHost                                                                                                                                                                            | `""`                                                                      |
| `kubectlImage.repository`                          | Repository for the kubectl image                                                                                                                                                                                                                             | `bitnamilegacy/kubectl`                                                   |
| `kubectlImage.tag`                                 | Tag for the kubectl image                                                                                                                                                                                                                                    | `1.30`                                                                    |
...
| `scratchVolume.fixerInitContainerImage.registry`   | Registry for the mode-fixer initContainer image. Empty means take it from global.imageRegistryHost                                                                                                                                                           | `""`                                                                      |
| `scratchVolume.fixerInitContainerImage.repository` | Repository for the mode-fixer initContainer image                                                                                                                                                                                                            | `library/alpine`                                                          |
| `scratchVolume.fixerInitContainerImage.tag`        | Tag for the mode-fixer initContainer image                                                                                                                                                                                                                   | `3.21`                                                                    |
...
| `anchoreConfig.component_catalog.server`                                                                   | Server configuration for the component catalog. Falls back to anchoreConfig.server if empty.                                                                          | `{}`                        |
| `anchoreConfig.component_catalog.logging`                                                                  | Logging configuration for the component catalog. Falls back to anchoreConfig.logging if empty.                                                                        | `{}`                        |
...
| `anchoreConfig.ui.log_format`                                                                              | Rendering of the application log output                                                                                                                               | `text`                      |
...
| Name                           | Description                                                                                                                                                                  | Value                                                                     |
| ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| `ui.image.registry`            | Registry for the Anchore UI image. Empty means take it from global.imageRegistryHost; set it to pull this one image from a different registry                                | `""`                                                                      |
| `ui.image.repository`          | Repository for the Anchore UI image                                                                                                                                          | `anchore/enterprise-ui`                                                   |
| `ui.image.digest`              | Digest for the Anchore UI image. Used when ui.image.tag is not set                                                                                                           | `sha256:6e1beda2983f3a9c2b9ca69fd1275bd3f403f5d6df562425ab75b75adc408af4` |
| `ui.image.tag`                 | Tag for the Anchore UI image. Set it to deploy a specific version instead of the digest the chart pins; quote the value so it is read as a string                            | `""`                                                                      |
...
| Name                                    | Description                                                                                                                                                                                                                | Value  |
| --------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| `preInstallJob.enabled`                 | Run the pre-install hook job that verifies database connectivity before installing. Disable if the chart cannot construct the DB connection (e.g. credentials injected as files rather than env vars via a Vault sidecar). | `true` |
...
| `cloudsql.image.registry`        | Registry for the GCE CloudSQL Proxy image. Stated because this is not a Docker Hub image, so global.imageRegistryHost does not apply to it. Override it to pull the proxy from a mirror | `gcr.io`                    |
| `cloudsql.image.repository`      | Repository for the GCE CloudSQL Proxy image                                                                                                                                             | `cloudsql-docker/gce-proxy` |
| `cloudsql.image.tag`             | Tag for the GCE CloudSQL Proxy image                                                                                                                                                    | `1.37.8`                    |
...
| `osaaMigrationJob.rbacCreate`                                | Create the ServiceAccount, Role and RoleBinding for the migration job's scale-down init container                                                                                                                                                | `true`                |
| `osaaMigrationJob.serviceAccountName`                        | Name of the service account for the migration job                                                                                                                                                                                                | `""`                  |
...
- When the upgrade job and the migration job are both creating RBAC for the same `serviceAccountName`, the ServiceAccount is now emitted once instead of twice. Both Roles and RoleBindings are still created, so the shared account holds the union of the permissions.
- Clarified `upgradeJob.rbacCreate`, `upgradeJob.serviceAccountName`, `osaaMigrationJob.rbacCreate`, and `osaaMigrationJob.serviceAccountName`. With `rbacCreate: true` the chart creates and owns the ServiceAccount, Role, and RoleBinding, and `serviceAccountName` names the ServiceAccount it creates. With `rbacCreate: false` the chart creates nothing and the account the job runs as is assumed to already carry the required permissions.

> :exclamation: If you set `upgradeJob.serviceAccountName` or `osaaMigrationJob.serviceAccountName` while the matching `rbacCreate` is `true`, and that ServiceAccount is managed outside this chart (for example an IRSA account created by eksctl or Terraform), set `rbacCreate: false` before upgrading. These are Helm hook resources and are deleted and recreated on every upgrade, so the chart would replace your ServiceAccount with one that does not carry your annotations. With `rbacCreate: false` the chart leaves it alone and assumes it already has permission to get/list/watch/update/patch deployments, patch deployments/scale, and get/list/watch pods.
...
- Rendered image references now always include the registry host, so `anchore/enterprise:v6.2.0` renders as `docker.io/anchore/enterprise:v6.2.0`. This resolves to the same image and is a no-op for pulls, but it changes the pod spec, so an upgrade will show a diff and roll the pods.
...
- Update upgrade job db connection logic check to use exit code instead of checking stderr
...
- Deploys Anchore Enterprise v6.2.0. See the [Release Notes](https://docs.anchore.com/current/docs/releasenotes/611/) for more information.
