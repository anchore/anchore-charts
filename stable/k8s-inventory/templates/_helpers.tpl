...
{{- else }}
  ...
  {{/* tag before digest, matching the enterprise chart: if a default ever pins a digest, Helm
       merges a user's `tag` into that default rather than replacing it, so digest-precedence
       would silently ignore the tag */}}
...
{{/*
The image used by the K8s Inventory deployment. This chart takes the dict form only -
image.pullPolicy lives in the same dict, so a bare reference string was never a valid
value here. An empty tag is an error rather than a fallback: the chart appVersion is
`1.8.5` while the published tags are `v1.8.5`, so defaulting to it would render a tag
that does not exist.
*/}}
{{- define "k8sInventory.image" -}}
{{- include "k8sInventory.renderImage" (dict "image" .Values.image "context" .) | trim -}}
{{- end }}
